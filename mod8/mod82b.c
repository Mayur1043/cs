...
//module_param_cb(myint, int, &myint, S_IRUGO | S_IWUSR);
//MODULE_PARM_DESC(myint, "This is a myint variable with callback");
...
static int wait_func(void *pv)
{
	while(1)
	{
		...
		{
			case  COMPLETION_READ:
				pr_info("%s: Message from Read: self-exiting!\n", __func__);				
				do_exit(0);
			case  COMPLETION_WRITE:
				pr_info("%s: Message from write: self-exiting!\n", __func__);				
				do_exit(0);
			case  COMPLETION_EXIT:
				pr_info("%s: Message from exit: self-exiting!\n", __func__);				
				do_exit(0);
		}
		/*else if (completion_flag == COMPLETION_WRITE)
		{
			pr_info("%s: Message from write: self-exiting!\n", __func__);
			do_exit(0);
		}
		else if (completion_flag == COMPLETION_READ)
		{
			pr_info("%s: Message from read: self-exiting!\n", __func__);
			do_exit(0);
		}*/
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
{
	...
	pr_info("Sent write done message!\n");
	if (!completion_done(&read_done))
	{
		complete(&read_done);
	}
