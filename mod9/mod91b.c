...
static int __init my_mod_init(void)
{
	...
	// add node1
	list_add_tail(&node1.my_list, &my_head);
	
	struct my_node node2;
	node2.data = 20;
	INIT_LIST_HEAD(&node2.my_list);
	list_add_tail(&node2.my_list, &my_head);
	
	struct my_node node3 =  
	{
		.data=30,
		.my_list = LIST_HEAD_INIT(node3.my_list)
	};
	list_add_tail(&node3.my_list, &my_head);
	
	struct my_node node4 =  
	{
		.data=40,
		.my_list = LIST_HEAD_INIT(node4.my_list)
	};
	list_add_tail(&node4.my_list, &my_head);
	
	struct my_node node5 =  
	{
		.data=50,
		.my_list = LIST_HEAD_INIT(node5.my_list)
	};
	list_add_tail(&node5.my_list, &my_head);
